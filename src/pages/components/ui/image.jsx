...
const Image = React.forwardRef(
  (
    ...
    if (!parsed) {
      ...
      // Locally hosted images (/images/*, Supabase Storage) get no CDN
      // transform, so the cropping the transform path would have applied has
      // to happen in CSS here. Without object-fit the browser stretches the
      // image to whatever box it is given, which distorts it.
      //
      // focalPointX/Y (0-100) choose which part survives the crop; `scale`
      // zooms in when a subject needs to fill more of the frame.
        ...
        <img
          ...
          style={{
            ...
            ...(typeof scale === "number" && scale !== 1
              ? { transform: `scale(${scale})`, transformOrigin: objectPosition || "center" }
              : {}),
            ...imageProps.style,
          }}
