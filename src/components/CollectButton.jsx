...
export default function CollectButton({ userId, artistId, artistName, work, workRef }) {
  ...
  const toggle = async (e) => {
    ...
    try {
      ...
      // Previously swallowed. A rejected collect left the button unchanged
      // with no explanation — the same silent failure that made the follow
      // button appear broken.
      setError(String(err?.message || err) || "Could not update your collection.");
  ...
  return (
    <span className="inline-flex flex-col gap-1">
      <button
        ...
        className={`flex items-center gap-1.5 font-mono-caps text-[11px] transition-colors disabled:opacity-60 ${collected ? "text-primary" : "text-muted-foreground hover:text-primary"} ${!userId ? "opacity-50" : ""}`}
