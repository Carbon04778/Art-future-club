...
export default function ArticleForm({ user, article, onClose, onCreated }) {
  // URL of the image being viewed full size, or null. Editing side only —
  // the reader view is unchanged.
  ...
  // A publish date in the future hides the article from everyone. Declared
  // here, AFTER `form` exists — reading it above the useState that creates it
  // threw "Cannot access 'form' before initialization" and blanked the page.
  const scheduledAt = form.publish_date ? new Date(form.publish_date) : null;
  const isScheduled = !!scheduledAt && !isNaN(scheduledAt) && scheduledAt > new Date();
            ...
            {list.map((e) => (
              <div key={e.id} className="border border-border p-3">
                <div className="relative group">
                  {/* Editing-side only: the thumbnail is cropped to a tidy
                      16:9, so click to see the whole image full size and check
                      what was actually inserted. */}
                  <button
                    type="button"
                    onClick={() => setExpanded(e.preview || e.url)}
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 bg-background/90 backdrop-blur-sm flex items-center justify-center p-6" onClick={onClose}>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5 overflow-y-auto">
            ...
            <Section title="Publishing">
              <div><label className="font-mono-caps text-[11px] text-muted-foreground">Publish Date — to backdate or schedule</label>
                ...
                {/*
                  An active warning, not a static hint. A future publish_date
                  hides the article completely, and three articles were saved
                  with the wrong YEAR before anyone noticed — the piece simply
                  vanished with no explanation. The date picker makes that easy
                  to do by accident, so say plainly what will happen.
                */}
                {isScheduled ? (
                  <div className="mt-2 border border-yellow-600/50 bg-yellow-500/10 px-3 py-2">
                    <p className="text-xs text-yellow-600">
                      This article will be hidden until{" "}
                      <strong>{scheduledLabel}</strong>. Clear the date to publish it now.
                    </p>
                  </div>
                ) : (
                  <p className="mt-1 text-xs text-muted-foreground">
                    Leave blank to use the creation time. A future date hides the article until then.
                  </p>
                )}
              </div>
            ...
            <div className="flex flex-wrap items-center gap-4 pt-2 pb-6">
              <button type="submit" disabled={saving} className="flex items-center gap-2 bg-primary px-8 py-4 font-mono-caps text-[11px] text-primary-foreground hover:opacity-80 disabled:opacity-50">
                ...
                {/* Say what the button will actually do. "Publish Article" is
                    misleading when a future date means it will not appear. */}
                {isScheduled
                  ? (isEdit ? "Save as Scheduled" : "Schedule Article")
      ...
      {expanded && (
        <div
          className="fixed inset-0 z-[10000] flex items-center justify-center bg-background/95 p-6"
        ...
        >
          ...
          <button
            type="button"
            onClick={() => setExpanded(null)}
            className="absolute top-6 right-6 border border-border bg-background px-4 py-2 font-mono-caps text-[11px] text-foreground hover:border-primary hover:text-primary"
          >
            Close
          </button>
        </div>
